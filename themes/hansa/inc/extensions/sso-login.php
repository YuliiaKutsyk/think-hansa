...
function hansa_sso_verify_token( $token ) {
    ...
    $url = 'https://api.loyale.io/api/Customer/VerifyToken';
...
function hansa_sso_auth_user($body) {
	...
	if(!empty($users)) {
		...
		$user_id = 0;
        $customer_data = loyale_get_customer($customer_id);
        if($customer_data) {
	    	...
	    	if($user_id) {
                loyale_update_customer_meta($customer_id,'WordpressCustomerId','Wordpress Customer Id',$user_id);
    ...
    do_action('loyale_before_login', $user_login, $user);
	return $user_id;
}
