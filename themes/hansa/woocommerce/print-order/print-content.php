		...
		<table>
			...
			<tfoot>
				...
				if($redeem_amount) {
					...
					$ra_arr = array('redeem_amount' => $ra_data);
					$totals_arr = array_insert_after( $totals_arr, 'tax', $ra_arr);
				}
				if($total_points) {
					if($redeem_amount) {
						$key = 'redeem_amount';
					...
					$tp_data = array(
						'label' => 'Total points',
						'value' => '+' . $total_points . 'pts'
					);
					$tp_arr = array('total_points' => $tp_data);
					$totals_arr = array_insert_after( $totals_arr, $key, $tp_arr);
				}
				if ( $totals_arr ) :
